export default function SectionHeading({ eyebrow, title, titleId, description, align, tone }) {
    ...
    return (
        <div className={centered ? 'mx-auto max-w-2xl text-center' : 'max-w-2xl'}>
            {eyebrow && (
                <p className={`flex items-center gap-2 text-sm font-medium ${centered ? 'justify-center' : ''} ${dark ? 'text-zinc-400' : 'text-zinc-600'}`}>
            ...
            <h2
                id={titleId}
                className={`mt-2 text-3xl font-bold tracking-tight sm:text-4xl ${dark ? 'text-white' : 'text-zinc-950'}`}
            >
                {title}
            </h2>
            {description && (
